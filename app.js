...
function scheduleInsightAuto(delay=INSIGHT_DELAY){
  ...
  insightTimer=setTimeout(()=>{if(document.hidden||!document.querySelector('.daily-carousel'))return;if(modal||document.activeElement?.matches('input,textarea,select,[contenteditable="true"]')){scheduleInsightAuto(2500);return}const next=buildDailyInsights(state.currentGame);if(next.length<2)return;insightIndex=(insightIndex+1)%next.length;refreshInsightDeck()},wait);
}
...
root.addEventListener("touchend",event=>{if(!insightTouch||!event.target.closest(".daily-carousel"))return;const t=event.changedTouches[0],dx=t.clientX-insightTouch.x,dy=t.clientY-insightTouch.y;insightTouch=null;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)){const cards=buildDailyInsights(state.currentGame);insightIndex=(insightIndex+(dx<0?1:-1)+cards.length)%cards.length;refreshInsightDeck()}},{passive:true});
