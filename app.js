...
function createCloudApi() {
  ...
  return {
    ...
    updateProfile: (profileId,name,emoji,accent,pattern,featuredAward) => rpc("korova_admin_update_profile_v2",{p_code:roomCode,p_token:adminToken(),p_profile_id:profileId,p_name:name,p_emoji:emoji,p_accent:accent,p_pattern:pattern,p_featured_award:featuredAward||null}),
...
root.addEventListener("click", async (event) => {
  ...
  if (action === "open-edit-icon" || action === "edit-profile-identity") {const player=action==="open-edit-icon"?state.currentGame.players.find(p=>p.id===button.dataset.id):null,profile=action==="edit-profile-identity"?(state.knownPlayers||[]).find(p=>p.id===button.dataset.id):profileEntity(player);if(!profile?.id)return;profileEditIntent={profileId:profile.id,playerId:player?.id||null};editingPlayerId=null;editingProfileId=null;modal="profile-auth";render();setTimeout(()=>document.querySelector('#profile-pin-form input')?.focus(),0);return}
...
root.addEventListener("submit", (event) => {
  ...
  if(event.target.id==="edit-profile-form"){const profile=(state.knownPlayers||[]).find(x=>x.id===editingProfileId),name=new FormData(event.target).get("name")?.trim();if(!isAdmin()){modal="admin";render();showToast("Оформление профиля доступно организатору");return}if(!profile||!name)return;mutate(()=>api.updateProfile(profile.id,name,selectedEmoji,selectedAccent,selectedPattern,selectedAward),"Оформление профиля сохранено");}
