...
function buildDailyInsights(game){
  ...
  const forecast={key:"forecast",kicker:"Прогноз на игру",title:size?`Кто фаворит за столом из ${size} игроков`:"Кто сегодня фаворит",body:`<div class="classic-forecast"><div ${profileAttrs(top.p,"forecast-star")}><span class="profile-avatar">${esc(top.p.emoji)}</span><div><small>фаворит по истории</small><b>${esc(top.p.name)}</b><p>${top.exact?`партии с ${size} игроками`:"общая статистика"}</p></div><strong>${top.chance}%</strong></div><div class="forecast-podium">${forecasts.slice(0,3).map((x,i)=>`<div ${profileAttrs(x.p,"forecast-profile")}><i>${i+1}</i><span>${esc(x.p.emoji)} <b>${esc(x.p.name)}</b></span><strong>${x.chance}%</strong></div>`).join("")}</div><p class="deck-footnote">Достоверность: <b>${confidence}</b> · выборка ${top.games} игр</p></div>`};
