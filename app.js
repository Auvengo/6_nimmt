...
const PROFILE_PALETTES=[
  {id:"amber",name:"Янтарь",color:"#d7a916",soft:"#fff1ad"},{id:"emerald",name:"Изумруд",color:"#2f8b68",soft:"#d7f3e7"},
  {id:"coral",name:"Коралл",color:"#d95f52",soft:"#ffe0d9"},{id:"sky",name:"Небо",color:"#4f82c5",soft:"#dceaff"},
  {id:"plum",name:"Слива",color:"#8d62b3",soft:"#eadffc"},{id:"terracotta",name:"Терракота",color:"#c97743",soft:"#f8dfcf"},
  {id:"olive",name:"Олива",color:"#768d45",soft:"#e5edcf"},{id:"berry",name:"Ягода",color:"#b94f78",soft:"#f8dce8"}
];
const PROFILE_PATTERNS=[{id:"none",name:"Без узора"},{id:"dots",name:"Крапинки"},{id:"stripes",name:"Полосы"},{id:"diamonds",name:"Ромбики"},{id:"waves",name:"Волны"}];
const DEFAULT_PROFILE_PATTERNS=PROFILE_PATTERNS.filter(x=>x.id!=="none");
const LIMIT = 10;
const WIN_LINE = 66;
...
let tab = "game";
let modal = null;
let selectedEmoji = EMOJIS[0], selectedAccent="amber", selectedPattern="dots", selectedAward="";
let busy = false;
let toastTimer;
let lastStateHash = "";
let editingRoundId = null;
let editingPlayerId = null, editingProfileId = null, profileEditIntent = null;
...
function createCloudApi() {
  ...
  return {
    ...
    updateProfile: (profileId,name,emoji,accent,pattern,featuredAward) => rpc("korova_update_profile_identity",{p_code:roomCode,p_profile_id:profileId,p_name:name,p_emoji:emoji,p_accent:accent,p_pattern:pattern,p_featured_award:featuredAward||null}),
...
function createLocalApi() {
  ...
  return {
    ...
    updateProfile: async (profileId,name,emoji,accent,pattern,featuredAward)=>{const s=load(),pr=s.knownPlayers.find(x=>x.id===profileId);if(pr){Object.assign(pr,{name,emoji,accent,pattern,featuredAward:featuredAward||null})}for(const x of [s.currentGame,...s.archive].flatMap(g=>g.players||[]))if(x.profileId===profileId){x.name=name;x.emoji=emoji}return save(s)},
...
async function exportBackup(format){await ensureHistory();const stamp=new Date().toISOString().slice(0,10);if(format==="json"){downloadText(`korova-${roomCode}-${stamp}.json`,JSON.stringify({version:"6.5",exportedAt:new Date().toISOString(),room:state.room,knownPlayers:state.knownPlayers,currentGame:state.currentGame,archive:state.archive},null,2),"application/json");return}const rows=[["Дата","Игрок","Место","Очки","Раунды","Игроков"]];for(const g of state.archive)rankingFor(g).forEach((x,i)=>rows.push([(g.finishedAt||g.startedAt||"").slice(0,10),x.name,i+1,x.total,g.rounds.length,g.players.length]));const csv="\ufeff"+rows.map(r=>r.map(v=>`"${String(v).replaceAll('"','""')}"`).join(";")).join("\n");downloadText(`korova-${roomCode}-${stamp}.csv`,csv,"text/csv;charset=utf-8")}

function esc(value = "") {
  return String(value).replace(/[&<>'"]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" }[c]));
}
function profileEntity(entity){if(!entity)return{};const id=entity.profileId||entity.id;return (state?.knownPlayers||[]).find(p=>p.id===id)||entity}
function profileHash(value){let h=0;for(const ch of String(value||"profile"))h=(h*31+ch.codePointAt(0))>>>0;return h}
function profileVisual(entity){const base=profileEntity(entity),p=entity&&(Object.prototype.hasOwnProperty.call(entity,"accent")||Object.prototype.hasOwnProperty.call(entity,"pattern"))?{...base,...entity}:base,h=profileHash(p.id||p.name),palette=PROFILE_PALETTES.find(x=>x.id===p.accent)||PROFILE_PALETTES[h%PROFILE_PALETTES.length],pattern=PROFILE_PATTERNS.find(x=>x.id===p.pattern)||DEFAULT_PROFILE_PATTERNS[Math.floor(h/PROFILE_PALETTES.length)%DEFAULT_PROFILE_PATTERNS.length];return{p,palette,pattern}}
function profileAttrs(entity,extra=""){const v=profileVisual(entity);return `class="profile-surface ${extra}" data-profile-pattern="${v.pattern.id}" style="--profile-accent:${v.palette.color};--profile-soft:${v.palette.soft}"`}
function profileAvatar(entity,extra=""){const p=profileEntity(entity);return `<span class="profile-avatar ${extra}">${esc(p.emoji||"🐮")}</span>`}
function emojiPicker(){return `<div class="emoji-groups">${EMOJI_GROUPS.map(g=>`<section><small>${g.name}</small><div class="emoji-grid">${g.items.map(e=>`<button type="button" class="emoji-choice ${e===selectedEmoji?"selected":""}" data-action="emoji" data-emoji="${e}" aria-label="Выбрать ${e}">${e}</button>`).join("")}</div></section>`).join("")}</div>`}
function earnedAwards(profileId){const temp=temporaryTitles().filter(t=>t.holders.some(p=>p.id===profileId)).map(t=>({name:t.name,kind:"Текущий титул"})),permanent=permanentAwards(profileId).map(x=>({name:x.name,kind:"Достижение"}));return[...temp,...permanent].filter((x,i,a)=>a.findIndex(y=>y.name===x.name)===i)}
function selectedProfileAward(p){const value=p?.featuredAward||"";return value&&earnedAwards(p.id).some(award=>award.name===value)?value:""}
function completedRoundNumber(game=state?.currentGame){return game?.rounds?.length||0}
function nextRoundNumber(game=state?.currentGame){return completedRoundNumber(game)+1}
function browserRoundTitle(game){return game?.players?.length>=2||completedRoundNumber(game)?`Раунд ${nextRoundNumber(game)}`:"Соберите стадо"}
function completedRoundLabel(game){const n=completedRoundNumber(game);return n?`После раунда ${n}`:"До первого раунда"}
...
function renderTvEvent(game){const e=latestGameEvent(game);return e&&Date.now()<tvEventUntil?`<div class="tv-event-layer" role="status" aria-live="polite"><div class="tv-event ${e.tone}"><span>${e.icon}</span><div><small>После раунда ${e.round}</small><b>${e.title}</b><p>${esc(e.text)}</p></div></div></div>`:''}
...
function renderTv(){if(tvCelebrationGame&&Date.now()<tvCelebrationUntil){renderTvCelebration(tvCelebrationGame);return}const g=state.currentGame,r=ranking(g),last=g.rounds.at(-1),today=todayGames(),leader=r[0],tvRows=Math.max(1,Math.ceil(r.length/2)),tvDense=r.length>=9?" tv-dense":"";document.body.classList.add("tv-mode");root.innerHTML=`<main class="tv-shell"><header><div class="tv-brand"><span>🐮</span><div><b>Коровосчёт 006</b><small>Большое табло · комната ${roomCode}</small></div></div><div class="tv-tools"><span class="tv-live ${navigator.onLine?"":"offline"}">● ${navigator.onLine?"В эфире":"Нет связи"}</span><button data-action="copy-tv-link">Ссылка</button><button data-action="tv-fullscreen">Во весь экран</button><a href="?room=${roomCode}">Выйти</a></div></header>${!g.players.length?`<section class="tv-empty"><div>🎴</div><h1>Ждём игроков</h1><p>Добавьте участников с телефона или ноутбука.</p></section>`:g.rounds.length?`<section class="tv-stage${tvDense}" style="--tv-rows:${tvRows}"><div class="tv-heading"><div><small>Текущая партия</small><h1>${completedRoundLabel(g)}</h1></div><div><b>${leader?`${esc(leader.emoji)} ${esc(leader.name)}`:"—"}</b><small>сейчас впереди</small></div></div>${renderTvEvent(g)}<div class="tv-board">${r.map((x,i)=>`<article ${profileAttrs(x,`${i===0?"leader":""} ${x.total>=66?"over66":""}`)}><span class="tv-place">${i+1}</span><span class="tv-avatar profile-avatar">${esc(x.emoji)}</span><b>${esc(x.name)}</b><em>${last?`+${last.scores[x.id]??0}`:""}</em><strong>${x.total}</strong><small>${x.total>=66?"66+ · игра продолжается":"очков"}</small></article>`).join("")}</div><footer><span>В архиве: <b>${archiveTotal()}</b></span><span>Последнее обновление: <b>${new Date().toLocaleTimeString("ru-RU",{hour:"2-digit",minute:"2-digit"})}</b></span></footer></section>`:`<section class="tv-wait"><div><small>Состав готов · ${g.players.length} ${plural(g.players.length,"игрок","игрока","игроков")}</small><h1>Начинаем игру</h1><p>Прогноз перед первым раундом</p></div>${dailyCard(g)}<div class="tv-roster-shell"><small>Сегодня за столом</small><div class="tv-roster" style="--tv-roster-columns:${Math.min(g.players.length,10)}">${g.players.map(x=>`<span ${profileAttrs(x,"tv-roster-player")}><i class="profile-avatar">${esc(x.emoji)}</i><b>${esc(x.name)}</b></span>`).join("")}</div></div></section>`}</main>`;scheduleInsightAuto()}
function render() {
  if (!state) {
...
function buildDailyInsights(game){
  ...
  const forecastRows=pool.map(p=>{const all=careerFor(p.id),same=all.rows.filter(r=>!size||r.game.players.length===size),exact=Boolean(size&&same.length>=3),base=exact?same:all.rows,wins=base.filter(r=>r.won).length,games=base.length,sampleWeight=.55+.45*Math.min(1,games/10),raw=((wins+1.25)/(games+5))*sampleWeight;return{p,wins,games,raw,exact}}),qualified=forecastRows.filter(x=>x.games>=3),backup=forecastRows.filter(x=>x.games>=2),provisional=qualified.length<2,forecasts=[...(qualified.length>=2?qualified:backup.length>=2?backup:forecastRows)].sort((x,y)=>y.raw-x.raw||y.games-x.games),excluded=Math.max(0,pool.length-forecasts.length),sum=forecasts.reduce((total,x)=>total+x.raw,0)||1;
  forecasts.forEach(x=>x.chance=Math.max(1,Math.round(x.raw/sum*100)));
  const top=forecasts[0],confidence=provisional?"предварительная":top.games>=10?"высокая":top.games>=5?"средняя":"низкая";
  const forecast={key:"forecast",kicker:"Прогноз на игру",title:size?`Кто фаворит за столом из ${size} игроков`:"Кто сегодня фаворит",body:`<div class="classic-forecast"><div ${profileAttrs(top.p,"forecast-star")}><span class="profile-avatar">${esc(top.p.emoji)}</span><div><small>фаворит по истории</small><b>${esc(top.p.name)}</b><p>${top.exact?`${top.games} партий с ${size} игроками`:`${top.games} партий в общей статистике`}</p></div><strong>${top.chance}%</strong></div><div class="forecast-podium">${forecasts.slice(0,3).map((x,i)=>`<div ${profileAttrs(x.p,"forecast-profile")}><i>${i+1}</i><span>${esc(x.p.emoji)} <b>${esc(x.p.name)}</b></span><strong>${x.chance}%</strong></div>`).join("")}</div><p class="deck-footnote">Достоверность: <b>${confidence}</b> · победы + объём выборки · минимум 3 партии${excluded?` · без рейтинга новичков: ${excluded}`:""}</p></div>`};
  ...
  const rivalryCard=rivalry?{key:"rivalry",kicker:"Дуэль вечера",title:`${esc(rivalry.a.name)} против ${esc(rivalry.b.name)}`,body:`<div class="duel-feature-v64"><div ${profileAttrs(rivalry.a,"duel-person")} >${profileAvatar(rivalry.a)}<b>${esc(rivalry.a.name)}</b></div><div class="duel-scoreline-v64"><div><strong>${rivalry.h.aw}</strong><small>${plural(rivalry.h.aw,"победа","победы","побед")}</small></div><em>:</em><div><strong>${rivalry.h.bw}</strong><small>${plural(rivalry.h.bw,"победа","победы","побед")}</small></div></div><div ${profileAttrs(rivalry.b,"duel-person")} >${profileAvatar(rivalry.b)}<b>${esc(rivalry.b.name)}</b></div><p class="duel-meta">${rivalry.h.shared} ${plural(rivalry.h.shared,"общая встреча","общие встречи","общих встреч")} · ничьи ${rivalry.h.ties}</p></div><p class="duel-last">⚡ ${lastText}</p>`}:forecast;
...
function scheduleInsightAuto(delay=INSIGHT_DELAY){
  ...
  insightTimer=setTimeout(()=>{
    ...
    if(!document.querySelector('.daily-carousel'))return;
    if(modal||document.activeElement?.matches('input,textarea,select,[contenteditable="true"]')){scheduleInsightAuto(2500);return}
    const next=buildDailyInsights(state.currentGame);
    if(next.length<2)return;
    insightIndex=(insightIndex+1)%next.length;
    refreshInsightDeck();
  },wait);
}
...
function refreshInsightDeck(){const current=document.querySelector('.daily-carousel');if(!current){render();return}const holder=document.createElement('div');holder.innerHTML=dailyCard(state.currentGame).trim();const next=holder.firstElementChild;if(!next)return;current.replaceWith(next);scheduleInsightAuto()}
  ...
  return `
    <div class="game-view ${game.rounds.length?"active-game":""}"><section class="hero-panel">
      <div>
        ...
        <h1>${browserRoundTitle(game)}</h1>
        <p>${game.players.length < 2 ? "Добавьте от 2 до 10 игроков, чтобы начать." : game.rounds.length ? `${esc(leader.emoji)} ${esc(leader.name)} впереди после ${completedRoundNumber(game)}-го раунда — ${leader.total} очк.` : "Все готовы. Внесите штрафные очки раунда 1."}</p>
      </div>
...
function renderPlayerCard(player, rank, game) {
  ...
  return `<article ${profileAttrs(player,`player-card ${rank === 0 && game.rounds.length ? "leader" : ""} ${!game.rounds.length ? "editable" : ""}`)}>
    <button class="player-icon editable-icon profile-avatar" data-action="open-edit-icon" data-id="${player.id}" title="Изменить значок" aria-label="Изменить значок игрока ${esc(player.name)}">${esc(player.emoji)}<span>${icon("edit")}</span></button>
    ...
    <div class="total"><strong>${player.total}</strong><span>итого</span></div>
...
function renderArchiveGame(game, index) {
  ...
  return `<article class="archive-card" data-action="open-archive-game" data-id="${game.id}">
    <div class="archive-head">
      ...
      <div ${profileAttrs(winners[0],"winner-badge")}><span class="profile-avatar">${esc(winners[0]?.emoji || "🏆")}</span><div><small>${winners.length > 1 ? "Победители" : "Победитель"}</small><b>${winners.map(p => esc(p.name)).join(", ") || "—"}</b></div></div>${isAdmin()?`<button class="archive-delete" data-action="delete-archive" data-id="${game.id}">Удалить</button>`:""}
    </div>
    <ol class="archive-results">
      ${ranks.map((p, i) => `<li ${profileAttrs(p,p.total === best ? "winner" : "")}><span class="place">${i + 1}</span><span class="avatar profile-avatar">${esc(p.emoji)}</span><b>${esc(p.name)}</b><strong>${p.total}<small> очк.</small></strong></li>`).join("")}
    </ol>
  </article>`;
}
...
function renderStatistics() {
  ...
  const player=p=>`<button ${profileAttrs(p,"stat-player stat-player-open")} data-action="open-player-card" data-id="${p.profileId||""}"><span class="stat-rank">${p.rank}</span><span class="avatar profile-avatar">${esc(p.emoji)}</span><b>${esc(p.name)}</b></button>`;
  ...
  <div class="stats-mobile"><div class="mobile-podium">${ranked.slice(0,3).map((p,i)=>`<button data-action="open-player-card" data-id="${p.profileId||""}" ${profileAttrs(p,`place-${i+1}`)}><em class="podium-place">${i+1}</em><span class="profile-avatar">${esc(p.emoji)}</span><b>${esc(p.name)}</b><strong>${p.wins} <small>${plural(p.wins,"победа","победы","побед")}</small></strong><small>${p.winRate}% · ${p.games} игр</small></button>`).join("")}</div>${ranked.length>3?`<h3 class="other-players-title">Остальные игроки</h3><div class="mobile-ranking">${ranked.slice(3).map(p=>`<button ${profileAttrs(p,"mobile-rank-profile")} data-action="open-player-card" data-id="${p.profileId||""}"><span class="stat-rank">${p.rank}</span><i class="profile-avatar">${esc(p.emoji)}</i><b>${esc(p.name)}</b><small>${p.games} игр · ${p.winRate}% · среднее ${p.average}</small><strong>${p.wins} ${plural(p.wins,"победа","победы","побед")}</strong><em>›</em></button>`).join("")}</div>`:""}</div>
  ${renderRecordBook()}
...
function playerAnalytics(profileId){const c=careerFor(profileId),vals=c.rows.map(x=>x.total),mean=c.average,variance=vals.length?vals.reduce((s,v)=>s+(v-mean)**2,0)/vals.length:0;let rounds=0,points=0,narrowWins=0,comebacks=0,zeroTriple=false;for(const row of c.rows){const g=row.game,p=g.players.find(x=>x.profileId===profileId),rank=ranking(g),second=rank[1]?.total??rank[0]?.total;if(row.won&&second-row.total<=3)narrowWins++;if(!hasRoundDetails(g))continue;rounds+=g.rounds.length;points+=row.total;let z=0;for(const r of g.rounds){if((r.scores[p.id]??0)===0){z++;if(z>=3)zeroTriple=true}else z=0}if(row.won&&g.rounds.length>=2){const half=Math.max(1,Math.floor(g.rounds.length/2)),partial=g.players.map(x=>({id:x.id,total:g.rounds.slice(0,half).reduce((s,r)=>s+(r.scores[x.id]??0),0)})).sort((x,y)=>x.total-y.total);if(partial.at(-1)?.id===p.id)comebacks++}}const recent=vals.slice(-3),prev=vals.slice(-6,-3),improvement=recent.length===3&&prev.length===3?(prev.reduce((a,b)=>a+b,0)/3-recent.reduce((a,b)=>a+b,0)/3):0;return{...c,variance,avgRound:rounds?points/rounds:Infinity,narrowWins,comebacks,zeroTriple,improvement}}
function temporaryTitles(){
  const average=values=>values.length?values.reduce((sum,value)=>sum+value,0)/values.length:Infinity;
  const placeAfter=(game,profileId,roundCount)=>{
    const ranked=game.players.map(player=>({player,total:(game.rounds||[]).slice(0,roundCount).reduce((sum,round)=>sum+Number(round.scores?.[player.id]||0),0)})).sort((a,b)=>a.total-b.total||a.player.seat-b.player.seat);
    const index=ranked.findIndex(row=>row.player.profileId===profileId);
    return index<0?null:index+1;
  };
  const all=(state.knownPlayers||[]).map(p=>({p,m:playerAnalytics(p.id)})).filter(x=>x.m.games).map(x=>{
    const recent3=x.m.rows.slice(-3),recent6=x.m.rows.slice(-6),recentTotals3=recent3.map(row=>row.total),recentTotals6=recent6.map(row=>row.total);
    const recentAverage=average(recentTotals3),mean6=average(recentTotals6),recentVariance=recentTotals6.length?recentTotals6.reduce((sum,value)=>sum+(value-mean6)**2,0)/recentTotals6.length:Infinity;
    const detailedRecent=recent6.filter(row=>hasRoundDetails(row.game)&&row.game.rounds?.length),zeroRounds=detailedRecent.reduce((sum,row)=>sum+row.game.rounds.reduce((count,round)=>{const player=row.game.players.find(candidate=>candidate.profileId===x.p.id);return count+(player&&Number(round.scores?.[player.id]||0)===0?1:0)},0),0),roundsPlayed=detailedRecent.reduce((sum,row)=>sum+(row.game.rounds?.length||0),0);
    const comebackGames=recent6.filter(row=>hasRoundDetails(row.game)&&(row.game.rounds?.length||0)>=2),comebackGains=comebackGames.map(row=>{const half=Math.max(1,Math.floor(row.game.rounds.length/2)),mid=placeAfter(row.game,x.p.id,half),finish=placeAfter(row.game,x.p.id,row.game.rounds.length);return mid&&finish?mid-finish:0});
    return{...x,recentAverage,recentVariance,recentGames3:recent3.length,recentGames6:recent6.length,detailedRecentGames:detailedRecent.length,zeroShare:roundsPlayed?zeroRounds/roundsPlayed:0,zeroRounds,comebackGames:comebackGames.length,comebackAverage:average(comebackGains),photoRate:x.m.wins?x.m.narrowWins/x.m.wins:0};
  });
  if(!all.length)return[];
  const award=(name,desc,list,metric,mode='max')=>{
    const candidates=list.map(item=>({item,value:metric(item)})).filter(row=>Number.isFinite(row.value));
    if(!candidates.length)return{name,desc,holders:[]};
    const best=mode==='min'?Math.min(...candidates.map(row=>row.value)):Math.max(...candidates.map(row=>row.value));
    const holders=candidates.filter(row=>Math.abs(row.value-best)<1e-9).map(row=>row.item.p).filter((player,index,items)=>items.findIndex(item=>item.id===player.id)===index);
    return{name,desc,holders};
  };
  const eligible3=all.filter(x=>x.m.games>=3),eligible6=all.filter(x=>x.m.games>=6);
  const comebackEligible=all.filter(x=>x.comebackGames>=3&&x.comebackAverage>0),cleanEligible=all.filter(x=>x.detailedRecentGames>=3&&x.zeroRounds>0),photoEligible=all.filter(x=>x.m.wins>=3&&x.m.narrowWins>0);
  const duelBest=new Map();
  for(let i=0;i<all.length;i++)for(let j=i+1;j<all.length;j++){
    const a=all[i],b=all[j],h=headToHead(a.p.id,b.p.id);if(h.shared<5||h.aw===h.bw)continue;
    const winner=h.aw>h.bw?a:b,wins=Math.max(h.aw,h.bw),rate=wins/h.shared,current=duelBest.get(winner.p.id);
    if(!current||rate>current.duelRate)duelBest.set(winner.p.id,{...winner,duelRate:rate});
  }
  return[
    award("Укротитель коров","Лучшее среднее за последние 3 партии",eligible3,x=>x.recentAverage,'min'),
    award("Коровий магнит","Самое высокое среднее за последние 3 партии",eligible3,x=>x.recentAverage),
    award("Тёмная лошадка","Лучшее улучшение последних 3 игр относительно предыдущих 3",eligible6.filter(x=>x.m.improvement>0),x=>x.m.improvement),
    award("Коровьи горки","Самые переменчивые результаты за последние 6 партий",eligible6,x=>x.recentVariance),
    award("Коровий метроном","Самые стабильные результаты за последние 6 партий",eligible6,x=>x.recentVariance,'min'),
    award("Король камбэков","Лучший средний подъём от середины к финишу в последних 6 партиях",comebackEligible,x=>x.comebackAverage),
    award("Чистые копыта","Наибольшая доля нулевых раундов в последних 6 партиях",cleanEligible,x=>x.zeroShare),
    award("Мастер фотофиниша","Наибольшая доля побед с отрывом до 3 очков, минимум 3 победы",photoEligible,x=>x.photoRate),
    award("Гроза соперников","Лучший процент побед в личной паре при 5+ встречах",[...duelBest.values()],x=>x.duelRate)
  ];
}
...
function renderAwardsHub(){
  ...
  const rows=temporary?temps.map(t=>`<article class="title-row" title="${esc(t.desc)}"><div><b>${t.name}</b><small>${t.desc}</small></div><span>${t.holders.length?t.holders.map(p=>`${esc(p.emoji)} ${esc(p.name)}`).join(", "):"Пока не присвоен"}</span></article>`).join(""):PERMANENT_RULES.map(r=>{const holders=(state.knownPlayers||[]).filter(p=>permanentAwards(p.id).some(x=>x.name===r[0]));return`<article class="title-row" title="${esc(r[1])}"><div><b>${r[0]}</b><small>${r[1]}</small></div><span>${holders.length?holders.map(p=>`${esc(p.emoji)} ${esc(p.name)}`).join(", "):"Ещё никто"}</span></article>`}).join("");
  const note=temporary?`<div class="award-period-note"><span>↻</span><div><b>Пересчитываются после каждой партии</b><small>Текущая форма — последние 3–6 игр. Для сложных титулов действует минимальная выборка.</small></div></div>`:"";
  return`<details class="awards-shell" ${awardsOpen?"open":""}><summary><span><small>Зал наград</small><b>Номинации и достижения</b></span><em>Показать</em></summary><section class="awards-hub"><header><div class="award-tabs"><button class="${temporary?"active":""}" data-action="award-tab" data-tab-id="temporary">Временные</button><button class="${!temporary?"active":""}" data-action="award-tab" data-tab-id="permanent">Постоянные</button></div></header>${note}<div class="title-list">${rows}</div></section></details>`;
}
...
function finalRankChart(game){if(game.rounds.length<2)return`<div class="final-chart-empty">Для динамики нужно минимум два раунда</div>`;const m=finalGameMetrics(game),players=ranking(game);return`<div class="final-rank-timeline">${players.map((p,finalIndex)=>{const places=m.series[p.id]||[];return`<article ${profileAttrs(p,`rank-journey ${finalIndex===0?'winner':''}`)}><div class="journey-player"><span class="profile-avatar">${esc(p.emoji)}</span><b>${esc(p.name)}</b><small>финиш: ${finalIndex+1} место</small></div><div class="journey-steps" aria-label="Места ${esc(p.name)} по раундам">${places.map((place,i)=>`<span class="${place===1?'first ':''}${i===places.length-1?'last':''}" title="Раунд ${i+1}: ${place} место"><small>Р${i+1}</small><b>${place}</b></span>`).join('<i>→</i>')}</div></article>`}).join('')}</div>`}
function finalPlayerMetrics(game){const m=finalGameMetrics(game),r=ranking(game);return`<div class="final-player-metrics">${r.map((p,i)=>{const rounds=game.rounds.length,avg=rounds?Math.round(p.total/rounds*10)/10:0,max=Math.max(...game.rounds.map(x=>Number(x.scores[p.id]||0)));return`<article ${profileAttrs(p,"final-player-profile")}><span class="profile-avatar">${esc(p.emoji)}</span><div><b>${esc(p.name)}</b><small>${i+1} место · среднее ${avg} за раунд</small></div><strong>${p.total}</strong><em>лучшее место ${m.bestPlace[p.id]} · лидер ${m.leadRounds[p.id]} ${plural(m.leadRounds[p.id],'раунд','раунда','раундов')} · максимум +${max}</em></article>`}).join('')}</div>`}
...
function renderModal() {
  ...
  if(modal==="profile-auth")return shell(`<span class="eyebrow">Защищённое действие</span><h2>Введите PIN организатора</h2><p class="modal-text">Имя, персонаж, цвет, узор и витринная награда относятся ко всему профилю игрока.</p><form id="profile-pin-form"><label class="field-label">Шестизначный PIN</label><input class="text-input admin-pin" name="pin" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" autocomplete="one-time-code" required autofocus><button class="button primary large full" type="submit" ${busy?"disabled":""}>Подтвердить и редактировать</button></form>`);
  ...
  if (modal === "add") {
    ...
    return shell(`<span class="eyebrow">Состав партии</span><h2>Кто сегодня играет?</h2>${returning.length ? `<span class="field-label">Уже играл</span><div class="known-players">${returning.map(p => `<button ${profileAttrs(p,"known-player")} data-action="add-existing" data-id="${p.id}"><span class="profile-avatar">${esc(p.emoji)}</span><b>${esc(p.name)}</b><small>Добавить</small></button>`).join("")}</div><div class="or-divider"><span>или новый игрок</span></div>` : ""}<form id="player-form">
      ...
      <span class="field-label">Выберите персонажа</span>${emojiPicker()}
      <button class="button primary large full" type="submit" ${busy ? "disabled" : ""}>${icon("userPlus")} Добавить нового игрока</button></form>`);
  }
  if (modal === "edit-icon") {
    const player = editingPlayerId?state.currentGame.players.find(p => p.id === editingPlayerId):(state.knownPlayers||[]).find(p=>p.id===editingProfileId);
    if (!player) return "";
    const profile=profileEntity(player),earned=earnedAwards(profile.id);return shell(`<span class="eyebrow">Профиль игрока</span><h2>${esc(profile.name)}</h2><form id="edit-profile-form" class="profile-identity-form"><div ${profileAttrs({...profile,accent:selectedAccent,pattern:selectedPattern},"identity-preview")}><span class="profile-avatar">${esc(selectedEmoji)}</span><div><b>${esc(profile.name)}</b><small>${selectedAward?`🏅 ${esc(selectedAward)}`:"Персональная карта"}</small></div></div><label class="field-label">Имя</label><input class="text-input" name="name" value="${esc(profile.name)}" maxlength="24" required><span class="field-label">Персонаж</span>${emojiPicker()}<span class="field-label">Акцентный цвет</span><div class="profile-colors">${PROFILE_PALETTES.map(x=>`<button type="button" class="${x.id===selectedAccent?"selected":""}" style="--swatch:${x.color}" data-action="profile-accent" data-value="${x.id}"><i></i><span>${x.name}</span></button>`).join("")}</div><span class="field-label">Фоновый узор</span><div class="profile-patterns">${PROFILE_PATTERNS.map(x=>`<button type="button" ${profileAttrs({...profile,accent:selectedAccent,pattern:x.id},x.id===selectedPattern?"selected":"")} data-action="profile-pattern" data-value="${x.id}"><i></i><span>${x.name}</span></button>`).join("")}</div><span class="field-label">Витринная награда</span><div class="profile-awards"><button type="button" class="${!selectedAward?"selected":""}" data-action="profile-award" data-value="">Без награды</button>${earned.map(x=>`<button type="button" class="${x.name===selectedAward?"selected":""}" data-action="profile-award" data-value="${esc(x.name)}"><small>${x.kind}</small><b>🏅 ${esc(x.name)}</b></button>`).join("")}</div>${earned.length?"":`<p class="identity-note">Награда появится после первого титула или достижения.</p>`}<button class="button primary large full" type="submit">${icon("save")} Сохранить оформление</button></form>`);
  }
  if (modal === "edit") {
...
function canvasWrappedText(ctx,text,x,y,maxWidth,lineHeight,maxLines=3){const words=String(text).split(/\s+/),lines=[];let line='';for(const word of words){const test=line?`${line} ${word}`:word;if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=word;if(lines.length===maxLines-1)break}else line=test}if(line&&lines.length<maxLines)lines.push(line);if(lines.length===maxLines&&words.join(' ').length>lines.join(' ').length)lines[maxLines-1]=lines[maxLines-1].replace(/[.,;:!?]?$/,'…');lines.forEach((value,i)=>ctx.fillText(value,x,y+i*lineHeight));return y+lines.length*lineHeight}
function shareCanvasFile(canvas,fileName,title){canvas.toBlob(async blob=>{if(!blob)return;const file=new File([blob],fileName,{type:'image/png'});try{if(navigator.canShare?.({files:[file]})){await navigator.share({title,files:[file]});return}}catch(error){if(error?.name==='AbortError')return}const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=file.name;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);showToast('Карточка сохранена — её можно отправить')},'image/png')}
function shareWinnerPane(game,kind='summary'){if(kind==='summary'){shareFinalCard(game);return}const canvas=document.createElement('canvas');canvas.width=1080;canvas.height=1350;const ctx=canvas.getContext('2d'),isStory=kind==='story',items=isStory?finalStory(game):funAwards(game);ctx.fillStyle='#123e32';ctx.fillRect(0,0,1080,1350);ctx.fillStyle='#f4c84a';ctx.fillRect(64,64,952,224);ctx.fillStyle='#201e1b';ctx.font='900 62px Arial';ctx.fillText('Коровосчёт',112,154);ctx.font='700 38px Arial';ctx.fillText(isStory?'Сюжет партии':'Номинации партии',112,218);ctx.font='24px Arial';ctx.fillText(new Date(game.finishedAt||Date.now()).toLocaleDateString('ru-RU'),112,256);ctx.fillStyle='#fffaf0';ctx.fillRect(64,316,952,970);ctx.fillStyle='#6f5a18';ctx.font='800 23px Arial';ctx.fillText(isStory?'КАК ВСЁ ПРОИСХОДИЛО':'ШУТОЧНОЕ НАГРАЖДЕНИЕ',112,370);let y=408;for(const [index,item] of items.slice(0,isStory?3:6).entries()){const h=isStory?220:126;ctx.fillStyle=index%2?'#fff4c8':'#ffffff';ctx.fillRect(104,y,872,h-12);ctx.fillStyle='#201e1b';ctx.font='36px Arial';ctx.fillText(isStory?item.icon:['🏅','🎖️','🥇','🐮','🎯','✨'][index%6],128,y+52);ctx.font='800 29px Arial';ctx.fillText(isStory?item.title:item.title,188,y+48);if(isStory){ctx.font='24px Arial';ctx.fillStyle='#625b4c';canvasWrappedText(ctx,item.text,188,y+88,760,34,3)}else{ctx.font='800 25px Arial';ctx.fillText(`${item.player.emoji} ${item.player.name}`,188,y+82);ctx.font='21px Arial';ctx.fillStyle='#625b4c';ctx.fillText(item.desc,188,y+108)}y+=h}ctx.fillStyle='#123e32';ctx.font='700 24px Arial';ctx.fillText('Меньше коров — ближе победа',112,1245);shareCanvasFile(canvas,isStory?'korova-story.png':'korova-awards.png',isStory?'Сюжет партии':'Номинации партии')}
...
root.addEventListener("click", async (event) => {
  ...
  if(action==="insight-prev"){pauseInsightAuto();const cards=buildDailyInsights(state.currentGame);insightIndex=(insightIndex-1+cards.length)%cards.length;refreshInsightDeck();return}
  if(action==="insight-next"){pauseInsightAuto();const cards=buildDailyInsights(state.currentGame);insightIndex=(insightIndex+1)%cards.length;refreshInsightDeck();return}
  if(action==="insight-go"){pauseInsightAuto();insightIndex=Number(button.dataset.index)||0;refreshInsightDeck();return}
  ...
  if(action==="share-winner-pane"&&celebrationGame){shareWinnerPane(celebrationGame,winnerTab);return}
  ...
  if (action === "close-modal" && (button.classList.contains("modal-close") || !event.target.closest("[data-modal]"))) { if(modal==="import-rounds"){captureImportValues();if(hasImportValues()&&!confirm("Закрыть ввод? Заполненные результаты будут удалены."))return;clearImportDraft()} modal = null; editingRoundId = null; editingPlayerId = null; editingProfileId = null; profileEditIntent = null; render(); }
  if (["emoji","profile-accent","profile-pattern","profile-award"].includes(action)) {const field=document.querySelector('#player-name, #edit-profile-form [name="name"]'),name=field?.value||"";if(action==="emoji")selectedEmoji=button.dataset.emoji;if(action==="profile-accent")selectedAccent=button.dataset.value;if(action==="profile-pattern")selectedPattern=button.dataset.value;if(action==="profile-award")selectedAward=button.dataset.value||"";render();const input=document.querySelector('#player-name, #edit-profile-form [name="name"]');if(input){input.value=name;input.focus()}return}
  if (action === "add-existing") { mutate(() => api.addExistingPlayer(button.dataset.id), "Игрок добавлен в состав"); }
  if (action === "open-edit-icon" || action === "edit-profile-identity") {const player=action==="open-edit-icon"?state.currentGame.players.find(p=>p.id===button.dataset.id):null,profile=action==="edit-profile-identity"?(state.knownPlayers||[]).find(p=>p.id===button.dataset.id):profileEntity(player);if(!profile?.id)return;editingPlayerId=player?.id||null;editingProfileId=profile.id;selectedEmoji=profile.emoji||EMOJIS[0];const visual=profileVisual(profile);selectedAccent=profile.accent||visual.palette.id;selectedPattern=profile.pattern||visual.pattern.id;selectedAward=selectedProfileAward(profile);profileEditIntent=null;modal="edit-icon";render();return}
...
root.addEventListener("touchend",event=>{if(!insightTouch||!event.target.closest(".daily-carousel"))return;const t=event.changedTouches[0],dx=t.clientX-insightTouch.x,dy=t.clientY-insightTouch.y;insightTouch=null;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)){const cards=buildDailyInsights(state.currentGame);insightIndex=(insightIndex+(dx<0?1:-1)+cards.length)%cards.length;refreshInsightDeck()}else scheduleInsightAuto()},{passive:true});
root.addEventListener("touchcancel",()=>{insightTouch=null;scheduleInsightAuto()},{passive:true});
root.addEventListener("pointerover",event=>{if(event.pointerType==='mouse'&&event.target.closest('.daily-carousel'))clearInsightAuto()});
root.addEventListener("pointerout",event=>{if(event.pointerType!=='mouse')return;const deck=event.target.closest('.daily-carousel');if(deck&&!deck.contains(event.relatedTarget))scheduleInsightAuto()});
...
root.addEventListener("submit", (event) => {
  ...
  if(event.target.id==="profile-pin-form"){const pin=new FormData(event.target).get("pin"),intent=profileEditIntent;if(!intent)return;busy=true;render();api.adminLogin(pin).then(session=>{adminSession=session;localStorage.setItem(adminKey,JSON.stringify(adminSession));const profile=(state.knownPlayers||[]).find(p=>p.id===intent.profileId);if(!profile)throw new Error("Профиль не найден");editingPlayerId=intent.playerId;editingProfileId=profile.id;selectedEmoji=profile.emoji||EMOJIS[0];const visual=profileVisual(profile);selectedAccent=profile.accent||visual.palette.id;selectedPattern=profile.pattern||visual.pattern.id;selectedAward=selectedProfileAward(profile);profileEditIntent=null;modal="edit-icon";busy=false;render();showToast("PIN подтверждён")}).catch(error=>{busy=false;modal="profile-auth";render();showToast(error.message||"Неверный PIN","error")});return}
