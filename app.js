...
function selectedProfileAward(p){return p?.featuredAward||""}
...
function temporaryTitles(){
  const all=(state.knownPlayers||[]).map(p=>({p,m:playerAnalytics(p.id)})).filter(x=>x.m.games).map(x=>{
    const recent3=x.m.rows.slice(-3),recent6=x.m.rows.slice(-6),avg=rows=>rows.length?rows.reduce((s,r)=>s+r.total,0)/rows.length:Infinity;
    const recentAverage=avg(recent3),mean6=avg(recent6),recentVariance=recent6.length?recent6.reduce((s,r)=>s+(r.total-mean6)**2,0)/recent6.length:Infinity;
    const detailed3=recent3.filter(r=>hasRoundDetails(r.game)),roundCount=detailed3.reduce((s,r)=>s+(r.game.rounds?.length||0),0),recentAvgRound=detailed3.length===3&&roundCount?detailed3.reduce((s,r)=>s+r.total,0)/roundCount:Infinity;
    return{...x,recentAverage,recentVariance,recentAvgRound};
  });
  const eligible=all.filter(x=>x.m.games>=3);
  if(!all.length)return[];
  const one=(name,desc,list,sort)=>{if(!list.length)return{name,desc,holders:[]};const sorted=[...list].sort(sort),best=sorted[0],holders=sorted.filter(x=>sort(x,best)===0).map(x=>x.p);return{name,desc,holders}};
  const h2h=all.map(x=>{let net=0;for(const y of all)if(x!==y){const z=headToHead(x.p.id,y.p.id);net+=z.aw-z.bw}return{...x,net}});
  return[
    one("Укротитель коров","Лучшее среднее за последние 3 партии",eligible,(a,b)=>a.recentAverage-b.recentAverage),
    one("Коровий магнит","Самое высокое среднее за последние 3 партии",eligible,(a,b)=>b.recentAverage-a.recentAverage),
    one("Железное копыто","Больше всего сыгранных партий",all,(a,b)=>b.m.games-a.m.games),
    one("Снайпер шестого ряда","Самый низкий результат одной партии",all,(a,b)=>a.m.best-b.m.best),
    one("Победный галоп","Самая длинная серия побед",all,(a,b)=>b.m.bestStreak-a.m.bestStreak),
    one("Коровьи горки","Самые переменчивые результаты за последние 6 партий",eligible,(a,b)=>b.recentVariance-a.recentVariance),
    one("Коровий метроном","Самые стабильные результаты за последние 6 партий",eligible,(a,b)=>a.recentVariance-b.recentVariance),
    one("Тёмная лошадка","Лучшее улучшение последних трёх игр",eligible.filter(x=>x.m.improvement>0),(a,b)=>b.m.improvement-a.m.improvement),
    one("Мастер фотофиниша","Больше всего побед с разницей до 3 очков",all.filter(x=>x.m.narrowWins),(a,b)=>b.m.narrowWins-a.m.narrowWins),
    one("Экономист стада","Самое низкое среднее за раунд в последних 3 партиях",eligible.filter(x=>Number.isFinite(x.recentAvgRound)),(a,b)=>a.recentAvgRound-b.recentAvgRound),
    one("Гроза соперников","Лучший баланс личных встреч",h2h,(a,b)=>b.net-a.net)
  ];
}
