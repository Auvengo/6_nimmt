...
function dailyCard(){if(!state.archive.length)return"";const all=(state.knownPlayers||[]).map(p=>({p,c:careerFor(p.id)})).filter(x=>x.c.games);if(!all.length)return"";const streak=[...all].sort((x,y)=>y.c.streak-x.c.streak)[0],record=[...all].sort((x,y)=>x.c.best-y.c.best)[0];return `<aside class="daily-card"><span>🎴 Карточка дня</span><b>${esc(streak.p.emoji)} ${esc(streak.p.name)}</b><small>${streak.c.streak?`серия ${streak.c.streak} побед`:`ближайший ориентир — рекорд ${record.c.best}`}</small></aside>`}
...
function renderAwardsHub(){const temps=temporaryTitles();const rows=gamificationTab==="temporary"?temps.map(t=>`<article class="title-row" title="${esc(t.desc)}"><div><b>${t.name}</b><small>${t.desc}</small></div><span>${t.holders.length?t.holders.map(p=>`${esc(p.emoji)} ${esc(p.name)}`).join(", "):"Пока не присвоен"}</span></article>`).join(""):PERMANENT_RULES.map(r=>{const holders=(state.knownPlayers||[]).filter(p=>permanentAwards(p.id).some(x=>x.name===r[0]));return`<article class="title-row" title="${esc(r[1])}"><div><b>${r[0]}</b><small>${r[1]}</small></div><span>${holders.length?holders.map(p=>`${esc(p.emoji)} ${esc(p.name)}`).join(", "):"Ещё никто"}</span></article>`}).join("");return`<section class="awards-hub"><header><div><span class="eyebrow">Зал наград</span><h2>Номинации и достижения</h2></div><div class="award-tabs"><button class="${gamificationTab==="temporary"?"active":""}" data-action="award-tab" data-tab-id="temporary">Временные</button><button class="${gamificationTab==="permanent"?"active":""}" data-action="award-tab" data-tab-id="permanent">Постоянные</button></div></header><div class="title-list">${rows}</div></section>`}
...
function renderModal() {
  ...
  if(modal==="player-card"){const p=(state.knownPlayers||[]).find(x=>x.id===selectedProfileId);if(!p)return"";const c=playerAnalytics(p.id),titles=temporaryTitles().filter(t=>t.holders.some(x=>x.id===p.id)),permanent=permanentAwards(p.id),opponents=(state.knownPlayers||[]).filter(x=>x.id!==p.id&&state.archive.some(g=>g.players.some(y=>y.profileId===p.id)&&g.players.some(y=>y.profileId===x.id))),opp=opponents.find(x=>x.id===selectedOpponentId)||opponents[0],h=opp?headToHead(p.id,opp.id):null,next=c.games<25?25:c.games<50?50:100;return shell(`<section class="premium-profile"><div class="premium-glow"></div><header><div class="premium-avatar">${esc(p.emoji)}</div><div><span class="eyebrow">Личная карточка</span><h2>${esc(p.name)}</h2><small>${c.games?`${Math.round(c.wins/c.games*100)}% побед · ${c.games} игр`:"Новый игрок"}</small></div></header><div class="premium-badges">${titles.map(t=>`<button title="${esc(t.desc)}">✦ ${t.name}</button>`).join("")||"<span>Временный титул ещё впереди</span>"}</div><div class="premium-metrics"><div><b>${c.games}</b><small>игр</small></div><div><b>${c.wins}</b><small>побед</small></div><div><b>${c.average}</b><small>среднее</small></div><div><b>${c.best}</b><small>рекорд</small></div></div><div class="permanent-strip"><b>Постоянные достижения</b>${permanent.length?permanent.map(x=>`<span title="${esc(x.desc)}">🏅 ${x.name}</span>`).join(""): `<small>Пока нет · до следующей вехи ${Math.max(0,next-c.games)} игр</small>`}</div>${opponents.length?`<div class="premium-h2h"><h3>Личные встречи</h3><div class="opponent-list">${opponents.map(x=>`<button class="${opp?.id===x.id?"active":""}" data-action="select-opponent" data-id="${x.id}">${esc(x.emoji)} ${esc(x.name)}</button>`).join("")}</div>${h?`<div class="h2h"><b>${h.aw} ${esc(p.name)}</b><span>${h.shared} общих игр · ничьи ${h.ties}</span><b>${esc(opp.name)} ${h.bw}</b></div>`:""}</div>`:""}</section>`)}
  if (modal === "add") {
...
root.addEventListener("click", async (event) => {
  ...
  if(action==="award-tab"){gamificationTab=button.dataset.tabId;render();return}
