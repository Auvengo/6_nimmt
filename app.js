...
function gameEvents(game){
  ...
  for(const g of state.archive||[]){if(g.id===game.id)continue;historicLongest=Math.max(historicLongest,g.rounds?.length||0);for(const r of g.rounds||[])for(const v of Object.values(r.scores||{}))historicHeavy=Math.max(historicHeavy,Number(v)||0)}
...
function profileHistoryChart(p,c){const rows=c.rows.slice(-15);if(!rows.length)return`<div class="profile-no-data">Завершённых партий пока нет</div>`;const W=620,H=205,left=28,right=18,top=20,bottom=34,max=Math.max(...rows.map(x=>x.total),1),min=Math.min(...rows.map(x=>x.total)),x=i=>left+i*((W-left-right)/Math.max(1,rows.length-1)),y=v=>top+(v-min)*((H-top-bottom)/Math.max(1,max-min));return`<div class="profile-chart"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Последние результаты ${esc(p.name)}"><line x1="${left}" y1="${top}" x2="${W-right}" y2="${top}" class="profile-grid"/><line x1="${left}" y1="${H-bottom}" x2="${W-right}" y2="${H-bottom}" class="profile-grid"/><polyline points="${rows.map((r,i)=>`${x(i)},${y(r.total)}`).join(' ')}" fill="none" stroke="#176b55" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>${rows.map((r,i)=>`<circle cx="${x(i)}" cy="${y(r.total)}" r="${r.won?7:5}" fill="${r.won?'#f4c84a':'#176b55'}" stroke="#201e1b" stroke-width="2"/><text x="${x(i)}" y="${Math.max(12,y(r.total)-10)}" text-anchor="middle" class="profile-score">${r.total}</text>`).join('')}</svg><div class="profile-history-links">${rows.slice(-5).reverse().map(r=>`<button data-action="open-archive-game" data-id="${r.game.id}"><span>${r.won?'🏆':'🎴'} ${new Date(r.game.finishedAt||r.game.startedAt).toLocaleDateString('ru-RU',{day:'numeric',month:'short'})}</span><b>${r.total}</b></button>`).join('')}</div></div>`}
function profileLastChange(profileId,c){if(!c.rows.length)return'';const last=c.rows.at(-1),beforeGames=(state.archive||[]).filter(g=>g.id!==last.game.id),before=profileSnapshot(profileId,beforeGames),nowRank=profileOverallRank(profileId),beforeRank=profileOverallRank(profileId,beforeGames);const items=[`<span><small>Победы</small><b>${before.wins} → ${c.wins}</b></span>`,`<span class="${c.average<before.average?'good':c.average>before.average?'bad':''}"><small>Средний итог</small><b>${before.games?before.average:'—'} → ${c.average}</b></span>`];if(beforeRank.place&&nowRank.place&&beforeRank.place!==nowRank.place)items.push(`<span class="${nowRank.place<beforeRank.place?'good':'bad'}"><small>Место</small><b>${beforeRank.place} → ${nowRank.place}</b></span>`);if(last.total===c.best)items.push(`<span class="good"><small>Личный рекорд</small><b>${c.best} очк.</b></span>`);return`<section class="profile-change"><header><span>✨</span><div><small>Последняя партия</small><b>Что изменилось</b></div></header><div>${items.join('')}</div></section>`}
...
function profileProgress(c){const goals=[];for(const n of [25,50,100])if(c.games<n){goals.push({name:n===25?'Ветеран стада':n===50?'Старейшина стада':'Легенда коровника',value:c.games,max:n,text:`${c.games} из ${n} партий`});break}for(const n of [3,5])if(c.bestStreak<n){goals.push({name:n===3?'Три в ряд':'Пять в ряд',value:c.streak,max:n,text:`${c.streak} из ${n} побед подряд`});break}return`<div class="profile-progress">${goals.slice(0,2).map(g=>`<article><div><b>${g.name}</b><small>${g.text}</small></div><span><i style="width:${Math.min(100,Math.round(g.value/g.max*100))}%"></i></span></article>`).join('')||'<small>Основные игровые вехи уже достигнуты</small>'}</div>`}
function profileHeadToHead(p,opp,h){if(!opp||!h)return'';const shared=(state.archive||[]).filter(g=>g.players.some(x=>x.profileId===p.id)&&g.players.some(x=>x.profileId===opp.id)).sort((a,b)=>new Date(b.finishedAt)-new Date(a.finishedAt)),last=shared[0],lastText=last?(()=>{const a=last.players.find(x=>x.profileId===p.id),b=last.players.find(x=>x.profileId===opp.id),at=totalFor(a.id,last),bt=totalFor(b.id,last);return at===bt?`Последняя встреча: ничья ${at}:${bt}`:`Последняя встреча: ${at<bt?esc(p.name):esc(opp.name)} · ${at}:${bt}`})():'';const recent=shared.slice(0,5),rw=recent.filter(g=>{const a=g.players.find(x=>x.profileId===p.id),b=g.players.find(x=>x.profileId===opp.id);return totalFor(a.id,g)<totalFor(b.id,g)}).length,ow=recent.filter(g=>{const a=g.players.find(x=>x.profileId===p.id),b=g.players.find(x=>x.profileId===opp.id);return totalFor(b.id,g)<totalFor(a.id,g)}).length;return`<div class="duel-card enhanced"><div class="duel-names"><span>${esc(p.name)}</span><span>${esc(opp.name)}</span></div><div class="duel-scoreline"><div><strong>${h.aw}</strong><small>${plural(h.aw,'победа','победы','побед')}</small></div><em>:</em><div><strong>${h.bw}</strong><small>${plural(h.bw,'победа','победы','побед')}</small></div></div><p>${h.shared} ${plural(h.shared,'общая встреча','общие встречи','общих встреч')} · ничьи ${h.ties}</p>${lastText?`<small>${lastText}</small>`:''}${recent.length>=3?`<b class="recent-duel">Последние ${recent.length}: ${rw} — ${ow}</b>`:''}</div>`}
function renderPersonalCard(p){const c=profileSnapshot(p.id),rank=profileOverallRank(p.id),form=profileForm(c),titles=temporaryTitles().filter(t=>t.holders.some(x=>x.id===p.id)),permanent=permanentAwards(p.id),opponents=(state.knownPlayers||[]).filter(x=>x.id!==p.id&&state.archive.some(g=>g.players.some(y=>y.profileId===p.id)&&g.players.some(y=>y.profileId===x.id))),opp=opponents.find(x=>x.id===selectedOpponentId)||opponents[0],h=opp?headToHead(p.id,opp.id):null;return`<section class="premium-profile profile-v62"><div class="premium-glow"></div><header><div class="premium-avatar">${esc(p.emoji)}</div><div><span class="eyebrow">Личная статистика</span><h2>${esc(p.name)}</h2><strong>${c.wins} ${plural(c.wins,'победа','победы','побед')}</strong><small>${c.games?`${Math.round(c.wins/c.games*100)}% побед · ${c.games} игр · место ${rank.place} из ${rank.total}`:'Новый игрок'}</small></div></header><div class="premium-metrics"><div><b>${c.wins}</b><small>побед</small></div><div><b>${c.average}</b><small>среднее</small></div><div><b>${c.best}</b><small>рекорд</small></div><div><b>${c.podiums}</b><small>в тройке</small></div></div>${profileLastChange(p.id,c)}<section class="profile-form"><div class="compact-heading"><h3>Текущая форма</h3><small>последние ${form.recent.length} игр</small></div><div class="form-scores">${form.recent.map(r=>`<button class="${r.won?'won':''}" data-action="open-archive-game" data-id="${r.game.id}"><b>${r.total}</b><small>${r.place} место</small></button>`).join('')||'<small>Нет завершённых игр</small>'}</div>${form.enough?`<div class="form-verdict ${form.change>0?'good':form.change<0?'bad':''}"><span>${form.change>0?'↘':form.change<0?'↗':'→'}</span><div><b>${form.change>0?`Форма улучшилась на ${form.change}`:form.change<0?`Средний итог вырос на ${Math.abs(form.change)}`:'Форма стабильна'}</b><small>Сейчас ${form.recentAvg} · раньше ${form.previousAvg}. Меньше очков — лучше.</small></div></div>`:`<p class="sample-note">Для достоверного сравнения нужны минимум 3 недавние и 3 предыдущие игры.</p>`}</section><section class="profile-history"><div class="compact-heading"><h3>История результатов</h3><small>до 15 последних игр</small></div>${profileHistoryChart(p,c)}</section><section><div class="compact-heading"><h3>Сильные стороны</h3><small>только проверенные данные</small></div>${profileStrengths(p.id,c)}</section>${opponents.length?`<section class="premium-h2h"><div class="compact-heading"><h3>Личные встречи</h3><small>${opponents.length} соперников</small></div><label class="opponent-select-wrap"><span>Соперник</span><select id="opponent-select">${opponents.map(x=>`<option value="${x.id}" ${opp?.id===x.id?'selected':''}>${esc(x.emoji)} ${esc(x.name)}</option>`).join('')}</select></label>${profileHeadToHead(p,opp,h)}</section>`:''}<section><div class="compact-heading"><h3>Ближайшие достижения</h3><small>прогресс</small></div>${profileProgress(c)}</section><details class="profile-titles"><summary>Временные титулы <span>${titles.length}</span></summary><div>${titles.length?titles.map(t=>`<article title="${esc(t.desc)}"><b>✦ ${t.name}</b><small>${t.desc}</small></article>`).join(''):'<small>Титулы ещё впереди</small>'}</div></details><details class="profile-achievements"><summary>Постоянные достижения <span>${permanent.length}</span></summary><div>${permanent.length?permanent.map(x=>`<span title="${esc(x.desc)}">🏅 ${x.name}</span>`).join(''):'<small>Полученных достижений пока нет</small>'}</div></details></section>`}
