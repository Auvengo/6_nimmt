...
function temporaryTitles(){
  ...
  const award=(name,desc,list,metric,mode='max',format=null)=>{
    const candidates=list.map(item=>({item,value:metric(item)})).filter(row=>Number.isFinite(row.value));
    if(!candidates.length)return{name,desc,holders:[],value:null,valueLabel:""};
    ...
    return{name,desc,holders,value:best,valueLabel:format?format(best):""};
  };
  ...
  for(const player of all){
    const recentGames=player.m.rows.slice(-10).map(row=>row.game);let wins=0,ties=0,shared=0,opponents=0;
    for(const rival of all){if(rival.p.id===player.p.id)continue;const pairGames=recentGames.filter(game=>game.players.some(candidate=>candidate.profileId===rival.p.id));if(pairGames.length<5)continue;opponents++;
      for(const game of pairGames){const a=game.players.find(candidate=>candidate.profileId===player.p.id),b=game.players.find(candidate=>candidate.profileId===rival.p.id);if(!a||!b)continue;const at=totalFor(a.id,game),bt=totalFor(b.id,game);shared++;if(at<bt)wins++;else if(at===bt)ties++;}
    }
    if(opponents>=2&&shared>=10)duelScores.push({...player,duelRate:(wins+ties*.5)/shared,duelWins:wins,duelTies:ties,duelShared:shared,duelOpponents:opponents});
  }
  return[
    ...
    award("Гроза соперников","Лучший процент парных побед в последних 10 играх: минимум 2 соперника и 5 встреч с каждым",duelScores,x=>x.duelRate,'max',value=>`${Math.round(value*100)}% парных побед`)
  ];
}
