...
function buildRecordBook(){
 ...
 low&&{icon:"🎯",name:"Рекорд комнаты",value:`${low.value} очк.`,who:`${low.player.emoji} ${low.player.name}`,desc:"Самый низкий итоговый штрафной счёт",game:low.game},
 big&&{icon:"💥",name:"Крупнейший отрыв",value:`+${big.value}`,who:`${big.player.emoji} ${big.player.name}`,desc:`Впереди ${big.runner.name}`,game:big.game},
 close&&{icon:"📸",name:"Самый близкий финиш",value:`+${close.value}`,who:`${close.player.emoji} ${close.player.name}`,desc:`Фотофиниш с ${close.runner.name}`,game:close.game},
 long&&{icon:"⏱️",name:"Самая длинная партия",value:`${long.value} р.`,who:`${long.player.emoji} ${long.player.name}`,desc:"Победитель марафона",game:long.game},
 roundHit&&{icon:"🐮",name:"Самый тяжёлый раунд",value:`+${roundHit.value}`,who:`${roundHit.player.emoji} ${roundHit.player.name}`,desc:"Максимальный штраф за один раунд",game:roundHit.game},
 veteran&&{icon:"🛡️",name:"Главный ветеран",value:`${veteran.c.games}`,who:`${veteran.p.emoji} ${veteran.p.name}`,desc:"Больше всего сыгранных партий"}
 ].filter(Boolean)}
...
function renderStatistics() {
  ...
  <div class="stats-mobile"><div class="mobile-podium">${ranked.slice(0,3).map((p,i)=>`<button data-action="open-player-card" data-id="${p.profileId||""}" class="place-${i+1}"><span>${esc(p.emoji)}</span><b>${esc(p.name)}</b><strong>${p.winRate}%</strong><small>${p.wins} ${plural(p.wins,"победа","победы","побед")}</small></button>`).join("")}</div><div class="mobile-ranking">${ranked.map(p=>`<button data-action="open-player-card" data-id="${p.profileId||""}"><span class="stat-rank">${p.rank}</span><i>${esc(p.emoji)}</i><b>${esc(p.name)}</b><small>${p.games} игр · среднее ${p.average}</small><strong>${p.winRate}%</strong><em>›</em></button>`).join("")}</div></div>
  ${renderRecordBook()}
