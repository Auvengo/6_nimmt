...
const PROFILE_PATTERNS=[{id:"dots",name:"Крапинки"},{id:"stripes",name:"Полосы"},{id:"diamonds",name:"Ромбики"},{id:"waves",name:"Волны"}];
const LIMIT = 10;
const WIN_LINE = 66;
...
let editingPlayerId = null, editingProfileId = null;
...
root.addEventListener("click", async (event) => {
  ...
  if (action === "open-edit-icon" || action === "edit-profile-identity") {if(!isAdmin()){modal="admin";render();showToast("Оформление профиля доступно организатору");return}const player=action==="open-edit-icon"?state.currentGame.players.find(p=>p.id===button.dataset.id):null,profile=action==="edit-profile-identity"?(state.knownPlayers||[]).find(p=>p.id===button.dataset.id):profileEntity(player);editingPlayerId=player?.id||null;editingProfileId=profile?.id||player?.profileId||null;selectedEmoji=profile?.emoji||EMOJIS[0];const visual=profileVisual(profile);selectedAccent=profile?.accent||visual.palette.id;selectedPattern=profile?.pattern||visual.pattern.id;selectedAward=profile?.featuredAward||"";modal="edit-icon";render();return}
