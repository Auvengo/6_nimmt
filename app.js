...
root.addEventListener("click", async (event) => {
  ...
  if(action==="toggle-import-mode"){const quick=button.dataset.mode==="quick";if(manualImport.quick!==quick){manualImport.quick=quick;manualImport.roundCount=quick?1:5;manualImport.roundIndex=0;manualImport.values={}}render();return}
  ...
  if(action==="import-add-round"){manualImport.roundCount++;manualImport.roundIndex=manualImport.roundCount-1;render();return}
  if(action==="import-round-prev"){manualImport.roundIndex=Math.max(0,(manualImport.roundIndex||0)-1);render();return}
  if(action==="import-round-next"){manualImport.roundIndex=Math.min(manualImport.roundCount-1,(manualImport.roundIndex||0)+1);render();setTimeout(()=>document.querySelector(".import-mobile-round input")?.focus(),0);return}
  if(action==="import-remove-round"&&manualImport.roundCount>1){const n=manualImport.roundCount-1;for(const k of Object.keys(manualImport.values||{}))if(k.startsWith(`r${n}-`))delete manualImport.values[k];manualImport.roundCount=n;manualImport.roundIndex=Math.min(manualImport.roundIndex||0,n-1);render();return}
...
root.addEventListener("submit", (event) => {
  ...
  if(event.target.id==="import-setup-form"){const fd=new FormData(event.target),ids=fd.getAll("profile"),date=fd.get("date");if(ids.length<2){showToast("Выберите минимум двух игроков","error");return}manualImport={date,profileIds:ids,roundCount:1,roundIndex:0,quick:true,values:{}};modal="import-rounds";render();return}
  if(event.target.id==="import-game-form"){const rounds=[];for(let i=0;i<manualImport.roundCount;i++){const row={};let any=false,all=true;for(const id of manualImport.profileIds){const v=manualImport.values[`r${i}-${id}`]??"";if(v!==""){any=true;row[id]=Number(v)}else all=false}if(any&&!all){showToast(`Заполните весь раунд ${i+1}`,"error");manualImport.roundIndex=i;render();return}if(any)rounds.push(row)}if(!rounds.length){showToast(manualImport.quick?"Заполните итоги":"Добавьте хотя бы один раунд","error");return}const mode=manualImport.quick?"totals":"rounds";mutate(()=>api.importGame(`${manualImport.date}T12:00:00`,manualImport.profileIds,rounds,mode),"Прошлая партия добавлена");return}
