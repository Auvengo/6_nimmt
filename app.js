...
function gameEvents(game){
  ...
  for(const g of state.archive||[]){historicLongest=Math.max(historicLongest,g.rounds?.length||0);for(const r of g.rounds||[])for(const v of Object.values(r.scores||{}))historicHeavy=Math.max(historicHeavy,Number(v)||0)}
